 ...
 *                      of a TreeModel
 */
QString RecordsManager::createQStringForGrants(int startYear, int endYear, std::vector<std::string> sortFields, char filterStart, char filterEnd) {
 ...
 *                      of a TreeModel
 */
QString RecordsManager::createQStringForPres(int startYear, int endYear, std::vector<std::string> sortFields, char filterStart, char filterEnd) {
 ...
 *                      of a TreeModel
 */
QString RecordsManager::createQStringForPubs(int startYear, int endYear, std::vector<std::string> sortFields, char filterStart, char filterEnd) {
    return QString(analyze(startYear, endYear, sortFields, sortFields.back(), filterStart, filterEnd).c_str());
}
 ...
 *                      of a TreeModel
 */
QString RecordsManager::createQStringForTeaching(int startYear, int endYear, std::vector<std::string> sortFields, char filterStart, char filterEnd) {
 ...
 */
std::vector<std::pair<std::string, int>> RecordsManager::getCountTuple(int startYear, int endYear, std::vector<std::string> headers, std::vector<std::string> matchStrings, char filterStart, char filterEnd) {
    std::vector<std::pair < std::string, int>> ret;

    // query the database
    std::string query = analyze(startYear, endYear, headers, headers.back(), filterStart, filterEnd);

    int pos = 0, depth = 0, index, totalCount;
    ...
    while ((index = query.find('\t', pos)) != -1) {
        ...
        if (frontValue.compare(matchStrings[depth]) == 0) {
            if (depth < (int) matchStrings.size() - 1) {
 ...
 */
std::vector<std::pair<std::string, double>> RecordsManager::getTotalsTuple(int startYear, int endYear, std::vector<std::string> headers, std::vector<std::string> matchStrings, std::string accCol, char filterStart, char filterEnd) {
...
std::string RecordsManager::removeTrailingZeros(double d) {
    std::string s;
    int strlen = std::snprintf(0, 0, "%.3f", d);
    s.resize(strlen + 1);

    std::snprintf(&s[0], strlen + 1, "%.3f", d);
    s.pop_back();
 ...
 */
std::string RecordsManager::analyze(int startYear, int endYear, std::vector<std::string> sortFields, std::string countCol, char filterStart, char filterEnd) {
 ...
 * @return              a formatted string with the analysis, can be used for TreeModel
 */
std::string RecordsManager::analyze(int startYear, int endYear, std::vector<std::string> sortFields, std::vector<std::string> accs,
    ...
    // analyze from there
    ret = analyze(sortedTree, sortFieldIndices, m, accsIndices, n, accsTotals, currencyMask, countColIndex, &sum, 0);
 ...
 * @param nFields       the number of fields to sort by
 * @param accs          the header indices of the fields to accumulate at
 * @param nAccs         the number of accumulators
 ...
 * @return              a formatted string with the analysis, can be used for TreeModel
 */
std::string RecordsManager::analyze(StringTree sortedTree, int sortFields[], int nFields, int accs[], int nAccs, double accsTotals[],
    ...
    for (StringTree::iterator sortField = sortedTree.begin(); sortField != sortedTree.end(); sortField = uniqueValue.second) {
        ...
        memset(accsTotal, 0, sizeof (accsTotal));
        ...
        if (nFields > depth) {
            returnString = analyze(newSorted, sortFields, nFields, accs, nAccs, accsTotal, currencyMask, countCol, &fieldCount, depth + 1);
        }
